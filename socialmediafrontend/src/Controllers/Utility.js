...
Utility.copy = (mainObj) =>{
  ...
  Utility.SearchPeople = async (value) =>{
    var SearchResult = [];
 ...
 await   Axios.post("http://localhost:3000/searchpeople",doc).then(response =>{
        console.log(response.data);
        if(response.data.length === 0){
            SearchResult.push( <li class='SearchDiv_li'>No Results Found</li>); 
        }else{
            
            response.data.forEach(element => {
                SearchResult.push(<Link to={"/viewprofile/"+element.id+"&"+element.email}><li id={element.id}  class='SearchDiv_li'>{element.FirstName} {element.LastName}</li></Link>)
            });
